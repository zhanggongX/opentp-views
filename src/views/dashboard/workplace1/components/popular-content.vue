<template>
  <a-spin :loading="loading" style="width: 100%">
    ...
    >
      <template #title>
        {{ $t('workplace1.popularContent') }}
      </template>
      <template #extra>
        <a-link>{{ $t('workplace1.viewMore') }}</a-link>
      </template>
      <a-space direction="vertical" :size="10" fill>
        ...
        >
          <a-radio value="text">
            {{ $t('workplace1.popularContent.text') }}
          </a-radio>
          <a-radio value="image">
            {{ $t('workplace1.popularContent.image') }}
          </a-radio>
          <a-radio value="video">
            {{ $t('workplace1.popularContent.video') }}
          </a-radio>
        </a-radio-group>
        <a-table
          :data="renderList"
